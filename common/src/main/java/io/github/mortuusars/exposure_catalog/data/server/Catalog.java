...
import io.github.mortuusars.exposure_catalog.network.packet.clientbound.SendExposureInfosPartS2CP;
...
public class Catalog {
    ...
    public static void send(Consumer<Packet> sender) {
        ...
        if (exposures.isEmpty()) {
            sender.accept(new SendExposureInfosPartS2CP(0, true, exposures));
        }
        else {
            List<List<ExposureInfo>> parts = Lists.partition(exposures, 2500);
            for (int i = 0; i < parts.size(); i++) {
                SendExposureInfosPartS2CP packet = new SendExposureInfosPartS2CP(i, i == parts.size() - 1, parts.get(i));
