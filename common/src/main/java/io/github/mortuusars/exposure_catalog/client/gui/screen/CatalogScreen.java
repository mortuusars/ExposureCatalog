...
public class CatalogScreen extends Screen {
    ...
    protected void init() {
        ...
        refreshButton = new ImageButton(leftPos + 7, topPos + 247, 12, 12, REFRESH_BUTTON_SPRITES, b -> refresh());
        ...
        exportButton = new ImageButton(leftPos + 26, topPos + 247, 12, 12, EXPORT_BUTTON_SPRITES, b -> exportExposures()) {
            @Override
    ...
    protected void filterSearchResults(Collection<String> items, String[] queries, Mode mode) {
        ...
        for (String query : queries) {
            ...
            if (mode == Mode.EXPOSURES && (query.startsWith("=") || query.startsWith("!="))) {
                ...
                SearchTree<String> tree = SearchTree.plainText(filtered, String::lines);
        ...
        for (int row = 0; row < ROWS; row++) {
            for (int column = 0; column < COLS; column++) {
                ...
                ExposureIdentifier identifier = mode == Mode.EXPOSURES
                        ...
                        : ExposureIdentifier.texture(ResourceLocation.parse(item));
    ...
    private List<Component> getThumbnailTooltipLines(Thumbnail thumbnail) {
        ...
        thumbnail.identifier().ifId(exposureId -> {
            ...
            if (exposureInfo != null) {
                ...
                if (!exposureInfo.paletteId().equals(ColorPalettes.DEFAULT.location())) {
                    lines.add(Component.translatable("gui.exposure_catalog.catalog.tooltip.palette",
                            exposureInfo.paletteId().toString()).withStyle(ChatFormatting.GRAY));
                }
